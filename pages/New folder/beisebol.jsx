...
const basketball = ({ featuredPosts, posts, tags, allUserTweets }) => {
  const [currentTime, setEventTime] = useState(eventTime[0]);
  const [currentSportEvent, setCurrentSportEvent] = useState(null);

  const { isLeagueLoading: loadingLeagues, data: leagues } = useQuery(
    "leagues",
    async () => {
  ...
  //   useEffect(() => {
  //     if (!isLoading) {
  //       console.log(Object.values(bevents?.events)[0]);
  //       setCurrentSportEvent(Object.values(bevents?.events)[0]);
  //     }
  //   }, [isLoading]);
  ...
  return (
    <>
      <Head>
        <title>Basket Ball | Bets</title>
      </Head>
      <Header />
      {/* {!isLoading && bevents.events.length != 0 && (
              ...
              <div className="col-md-5">
                <div className="common_dropdown tw-h-full tw-max-h-[450px] tw-overflow-hidden ">
                  ...
                  >
                    ...
                    {!isLoading &&
                      Object.values(bevents?.events)?.map((sportEvent) => {
                        return (
                          <ul
                            key={sportEvent.id}
