...
const sportEventSummary = async (router) => {
  ...
  let sport = Object.values(gameJson).filter((d) => d.route == spName);
  if (sport.length == 0) router.push("/");
  const queryValue =
    router.query["game"] ||
    router.asPath.match(new RegExp(`[&?]game=(.*)(&|$)`));
  const res = await fetch(
    `https://proxy.bets.com.br/${sport[0].en_Name}/trial/${sport[0].api_version}/pt/sport_events/${queryValue}/summary.json?api_key=${sport[0].api_key}`
  );
  return res.json();
};
const sportEventCommentry = async (router) => {
  ...
  const eventComentryRes = await fetch(
    `https://proxy.bets.com.br/${sport[0].en_Name}/trial/${sport[0].api_version}/pt/sport_events/${queryValue}/timeline.json?api_key=${sport[0].api_key}`
  );
...
export default function game() {
  ...
  useEffect(async () => {
    if (status == "success") {
      // const spName = localStorage.getItem("sportName");
      console.log("router.query", router.query.game);
      const spName = router.query.sportName;
      console.log("spName11111", spName);
      if (spName == "" || spName == undefined || spName == null) {
        // router.push("/");
      }
                ...
                {status === "success" && (
                  <>
                    <div className="game-title">
                      ...
                      <p>
                        {isValidElement(data.sport_event.venue) && (
                          <>
                    ...
                    <div className="row align-items-center">
                      <div className="col-md-4 col-4">
                        <div className="game-data">
                          ...
                          <div className="game-icon">
                            ...
                            {teamLogoImg(data.sport_event.competitors[0].name)}
                          </div>
                        </div>
                      </div>
                      <div className="col-md-4 col-4">
                      ...
                      <div className="col-md-4 col-4">
                        <div className="game-right">
                          <div className="game-icon">
                            {/* <img src={"/img/langu_team.png"} alt="game_icon" /> */}
                            {teamLogoImg(data.sport_event.competitors[1].name)}
                          </div>
